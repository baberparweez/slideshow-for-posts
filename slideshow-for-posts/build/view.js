...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".slideshow_for_posts--container").forEach(container => {
    ...
    const customApiUrl = container.getAttribute("data-custom-api-url") || "https://wptavern.com";
...
function renderPosts(posts, container) {
  let slidesOuter = container.querySelector(".slideshow_for_posts--slides_outer");
...
function fetchAndUpdatePosts(apiUrl, container, numberOfPosts, sortOrder) {
  ...
  if (cachedPosts) {
    ...
    fetch(`${apiUrl}/wp-json/wp/v2/posts?_embed&per_page=${numberOfPosts}&order=${sortOrder}&orderby=date`).then(response => response.json()).then(posts => {
...
function setupSlider(container) {
  ...
  let currentIndex = 0;
  const slidesContainer = container.querySelector(".slideshow_for_posts--slides");
  const slides = slidesContainer.querySelectorAll(".slideshow_for_posts--slide");
  ...
  if (slides.length > 1) {
    ...
    prevBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M12 2a10 10 0 1 0 10 10A10.011 10.011 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8.009 8.009 0 0 1-8 8z"/><path d="M13.293 7.293 8.586 12l4.707 4.707 1.414-1.414L11.414 12l3.293-3.293-1.414-1.414z"/></svg>';
    prevBtn.classList.add("slideshow_for_posts--prev");
    prevBtn.addEventListener("click", goToPreviousSlide);
    const nextBtn = document.createElement("button");
    nextBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M12 2a10 10 0 1 0 10 10A10.011 10.011 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8.009 8.009 0 0 1-8 8z"/><path d="M13.293 7.293 8.586 12l4.707 4.707 1.414-1.414L11.414 12l3.293-3.293-1.414-1.414z"/></svg>';
